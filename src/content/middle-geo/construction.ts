  ...
  steps: [
    {
      ...
      render: ({ scene, params }) => {
        ...
        g.add(createPoint(-2, 0, 0, NEON_GREEN, 0.12));
        g.add(createPoint(2, 0, 0, NEON_GREEN, 0.12));

        g.add(createCircle2D(-2, 0, r, NEON_ORANGE));
        g.add(createCircle2D(2, 0, r, NEON_MAGENTA));

        const halfAB = 2;
        if (r > halfAB * 0.5) {
          const h = Math.sqrt(r * r - halfAB * halfAB);
          if (isFinite(h)) {
            g.add(createPoint(0, h, 0, NEON_GREEN, 0.15));
            g.add(createPoint(0, -h, 0, NEON_GREEN, 0.15));

            g.add(createDashedLine(
            ...
            g.add(createLabel("수직이등분선", new THREE.Vector3(1.5, 4, 0), "#00ff88", 0.9));
          }
        }

        g.add(createLabel("A", new THREE.Vector3(-2.3, -0.5, 0), "#009E73", 0.8));
        g.add(createLabel("B", new THREE.Vector3(2.3, -0.5, 0), "#009E73", 0.8));
        g.add(createLabel("M", new THREE.Vector3(0.3, -0.5, 0), "#fff", 0.7));
    ...
    },
    {
      ...
      render: ({ scene }) => {
        ...
        g.add(createCircle2D(-2, -1, side, NEON_ORANGE));
        g.add(createCircle2D(2, -1, side, NEON_MAGENTA));

        const h = Math.sqrt(3) / 2 * side;
        const C = new THREE.Vector3(0, -1 + h, 0);
        g.add(createPoint(C.x, C.y, 0, NEON_GREEN, 0.15));
        ...
        g.add(createPoint(-2, -1, 0, NEON_GREEN, 0.12));
        g.add(createPoint(2, -1, 0, NEON_GREEN, 0.12));

        g.add(createLabel("A", new THREE.Vector3(-2.5, -1.5, 0), "#009E73", 0.8));
        g.add(createLabel("B", new THREE.Vector3(2.5, -1.5, 0), "#009E73", 0.8));
        g.add(createLabel("C", new THREE.Vector3(0.3, C.y + 0.3, 0), "#009E73", 0.8));
        g.add(createLabel("정삼각형", new THREE.Vector3(0, C.y + 1, 0), "#009E73"));
    ...
    },
    {
      ...
      render: ({ scene, params }) => {
        ...
        g.add(createPoint(P1.x, P1.y, 0, NEON_ORANGE, 0.1));
        g.add(createPoint(P2.x, P2.y, 0, NEON_ORANGE, 0.1));

        g.add(createCircle2D(P1.x, P1.y, 1.5, NEON_MAGENTA));
        g.add(createCircle2D(P2.x, P2.y, 1.5, NEON_GREEN));

        const bisAngle = rad / 2;
        const bisEnd = new THREE.Vector3(len * Math.cos(bisAngle), len * Math.sin(bisAngle), 0);
        g.add(createDashedLine(O, bisEnd, 0x00ff88, 0.2, 0.1));

        g.add(createLabel(`${deg}°`, new THREE.Vector3(1.5, 0.5, 0), "#0072B2", 0.8));
        g.add(createLabel(`${(deg / 2).toFixed(1)}°`, new THREE.Vector3(
          2 * Math.cos(bisAngle) + 0.3,
          2 * Math.sin(bisAngle),
          0,
        ), "#00ff88", 0.8));
        g.add(createLabel("이등분선", new THREE.Vector3(bisEnd.x - 1, bisEnd.y + 0.4, 0), "#00ff88"));
